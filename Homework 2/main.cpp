...
int main()
 {
    ...
    for (int i=1; i<=100; i++) //100 day simulation of office hours
    {
        ...
        for (int i=0; i<60; i++) //Simulation of 60 minutes of office hours
        {
            ...
            {
                //cout << "A student shows up" <<"\t\t";  //uncomment to show simulation
            }
            ...
            if (prob > (100-setProb))
            {
                ...
                q1.push(a);
                q2.push(i);
                //cout <<"Time needed: " << a.getTimeNeeded();  //uncomment to show simulation
        ...
        while (!q1.empty()) //calculating wait time
        {
            if (q1.size()>=1)
            {
                ...
                Student temp = q1.front();
                int studentTime = temp.getTimeNeeded();
                tempTime = tempTime + studentTime;
                q1.pop();
                q2.pop();
                if ((tempTime-1) < q2.front())
                {
                    tempTime = q2.front();
                }
                else if ((tempTime-1) >= q2.front())
                {
                    totalWaitTime = totalWaitTime + (tempTime - q2.front());
                }
            }
        }
    }
